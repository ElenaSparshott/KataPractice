...
//reverse the sum
// * individual numbers sum by the reversed number.
//return true is number is equal to original number.
...
function numberJoy(n) {
  let word = n.toString().split('')
  let num = word.map(x => Number(x))
  let reduced = num.reduce((a, b) => a + b, 0)
  let reversed = reversedNum(reduced)
  let product = reduced * reversed
  return product === n
}